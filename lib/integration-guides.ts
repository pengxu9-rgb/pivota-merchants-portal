...
const enGuides: Record<IntegrationGuideKey, IntegrationGuide> = {
  ...
  wix: {
    ...
    category: "sales_channels",
    name: "Wix",
    title: "Connect Wix with a Site ID or Wix URL and API key",
    summary:
      "Pivota connects to Wix with the Site ID from the Wix URL and an API key so product data can be validated through Wix Stores APIs.",
    requiredFields: [
      "Site ID from the Wix URL, or the full Wix URL if it contains the Site ID",
      "API key generated by the Wix site owner or account owner",
      "Optional store name for display inside Pivota",
    ],
    fieldMappings: [
      "Wix Site ID or a Wix URL containing that ID maps to Pivota Site ID.",
    ...
    steps: [
      "Open the target Wix site in Wix management.",
      "Copy the Site ID from the browser URL, commonly the UUID after /dashboard/.",
      ...
      "Paste the Site ID or full Wix URL and API Key into Pivota, add an optional store name, then connect.",
    ],
    pitfalls: [
      "A public storefront URL may only contain a vanity slug; use a Wix URL that includes the Site ID UUID, or paste the Site ID directly.",
    ...
    validationNotes: [
      ...
      "If Wix rejects the request, recheck ownership, Site ID or URL, and API key permissions.",
    ],
...
const zhGuides: Record<IntegrationGuideKey, IntegrationGuide> = {
  ...
  wix: {
    ...enGuides.wix,
    title: "用 Site ID 或 Wix URL 和 API key 连接 Wix",
    summary: "Pivota 使用 Wix URL 中的 Site ID 和 API key 连接 Wix，以便通过 Wix Stores API 验证商品数据。",
    requiredFields: ["Wix URL 中的 Site ID，或包含 Site ID 的完整 Wix URL", "由 Wix site owner 或 account owner 生成的 API key", "可选的 Store Name，用于 Pivota 内显示"],
    fieldMappings: ["Wix Site ID 或包含该 ID 的 Wix URL 对应 Pivota Site ID。", "Wix API key 对应 Pivota API Key。", "Store Name 是可选显示名。"],
    prerequisites: ["Wix 站点需要启用 Wix Stores。", "API key 必须由有目标站点权限的 owner 生成。", "API key 需要有读取产品或商店目录的权限。"],
    steps: ["在 Wix 管理端打开目标站点。", "从浏览器 URL 中复制 Site ID，通常是 /dashboard/ 后面的 UUID。", "打开 Wix API Keys Manager，为账号或站点创建 API key。", "给 key 授权读取 Wix Stores 商品数据所需权限。", "把 Site ID 或完整 Wix URL、API Key 填入 Pivota，可选填 Store Name，然后连接。"],
    pitfalls: ["Public storefront URL 可能只有 vanity slug；请使用包含 Site ID UUID 的 Wix URL，或直接粘贴 Site ID。", "错误账号生成或没有目标站点权限的 key 可能导致验证或商品查询失败。", "复制凭据时请区分测试站点和生产站点。"],
    validationNotes: ["Pivota 会用 API key 和 Site ID 发起一个小的 Wix Stores products query。", "如果 Wix 拒绝请求，请检查 owner、Site ID 或 URL、API key 权限。"],
  },
  woocommerce: {
