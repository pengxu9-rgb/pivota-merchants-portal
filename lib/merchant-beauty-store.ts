/**
 * Zustand store for the merchant beauty-field authoring thread.
 * Mirrors the structure of merchant-fashion-store but drives the beauty
 * completeness workflow (subcategory-aware fields via beauty_field_authoring.py v2.1).
 */
...
function persistName(): string {
  if (typeof window === "undefined") return "merchant-beauty-thread-v1-ssr";
  try {
    const raw = window.localStorage.getItem("merchant_user");
    if (!raw) return "merchant-beauty-thread-v1-anon";
    const parsed = JSON.parse(raw) as { merchant_id?: unknown } | null;
    const id =
      parsed && typeof parsed.merchant_id === "string" ? parsed.merchant_id : "anon";
    return `merchant-beauty-thread-v1-${id}`;
  } catch {
    return "merchant-beauty-thread-v1-anon";
  }
}
...
export const useMerchantBeautyStore = create<BeautyThreadState & Actions>()(
  persist(
    (set, get) => ({
      ...initialState,
      ...
      setQueue: (queue, totals) => {
        ...
        set({
          ...
          threadId: prev.threadId || `beauty_thread_${Date.now().toString(36)}`,
