...
class ApiClient {
  ...
  // ── W5 P4: request indexing of a product's Pivota canonical page (ADR-006 —
  // this submits agent.pivota.cc/products/… to Google Search Console, NEVER the
  // merchant's own store page). Both endpoints live under the merchant audit
  // router (/api/merchant-center/audit/...). The status dict is honest:
  // not_enabled (GSC submission not turned on yet) | no_canonical_url (nothing
  // to submit) | submitted | pending | indexed.
  async requestSkuIndexing(args: {
    targetSkuKey: string;
    auditRunId?: string | null;
  }): Promise<import('./types/ai-readiness').SkuIndexingStatusResponse> {
    const response = await this.client.post(
      '/api/merchant-center/audit/sku/request-indexing',
      {
        target_sku_key: args.targetSkuKey,
        ...(args.auditRunId ? { audit_run_id: args.auditRunId } : {}),
      },
    );
    return response.data?.data || response.data;
  }
